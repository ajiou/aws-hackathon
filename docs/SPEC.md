...
> ⚠ **本表已於 2026-09-12 依實測修正，見 [ADR-0001](adr/0001-風險分數的權重與缺資料處理.md)。**
> 現行權重如下表第二段。原表（違規 35/20/15/30、私立 50/28.6/21.4/—）保留在第一段供對照。

| 園所類型 | 違規 | 評鑑 | 輿情 | 營運 |
|---|--:|--:|--:|--:|
| 公立（原） | 35% | 20% | 15% | 30% |
| 非營利（原） | 35% | 20% | 15% | 30% |
| **私立（原）** | **50%** | **28.6%** | **21.4%** | 不適用 |
| 公立（現行） | 26.6% | 43.4% | **不計分** | 30% |
| 非營利（現行） | 26.6% | 43.4% | **不計分** | 30% |
| **私立（現行）**（835 園在營運） | **38%** | **62%** | **不計分** | 不適用 |

兩處改動的依據（完整量測見 ADR-0001）：**輿情維度單獨 Precision@50 = 8.0%，
低於隨機抽查的 10.9%**，且有輿情訊號的 11 區 867 園被罰率與全市無異；
**違規:評鑑 改為 38:62 是照 §6.3 第 4 列那條實測 28.0% 的基準線的隱含比例**
——該基準線 `裁罰次數 + 2×基礎評鑑未通過 + 3×行政處分` 中評鑑實際佔約 62%，
與本表原本給違規最大權重的設定矛盾。
...
> ⚠ **維度層級的分母已於 2026-09-12 修正，見 [ADR-0001](adr/0001-風險分數的權重與缺資料處理.md)。**
> 子指標層級照本節原文不變。維度層級的 `R_raw` 分母改為 `Σ(w_k)`，只算**適用**的維度：
> 「不適用」（私立沒有營運維度）退出分母，「有資料缺口」（137 園查無切點前評鑑紀錄）
> 留在分母、貢獻按覆蓋率打折。原文把兩者一視同仁，會讓查無評鑑紀錄的園由違規維度
> 單獨決定分數並被放大——實測那 101 園被罰率 7.9% 低於母體 10.9%，卻佔掉 top-50 的
> 11 個名額只命中 1 家。
...
> ⚠ **本維度已於 2026-09-12 移出風險分數，見 [ADR-0001](adr/0001-風險分數的權重與缺資料處理.md)。**
> 實測單獨 Precision@50 = 8.0%，低於隨機抽查的 10.9%；有 B 級訊號的 11 區共 867 園
> 被罰率 10.5%，與全市 10.9% 無異——L2 區級熱度買到的是覆蓋率，不是準確度。
> **資料與前端展示全部保留**，只是不計分。下面的三層設計仍是資料產出的規格。
>
> 另見 [ADR-0003](adr/0003-輿情特徵的時間切分.md)：`docs.json` 有 1,540 篇在切點之後，
> `park_risk.json` / `district_heat.json` 的 `as_of` 是 2026-08-23，
> **直接拿來當特徵就是用 2026 年的新聞預測 2025 年的裁罰**。特徵一律重算。
...
> ⚠ **門檻的量測方法與模型的量測方法不同，見 [ADR-0001](adr/0001-風險分數的權重與缺資料處理.md) 第四節。**
> 28.0% 是第 4 列那條**用原始次數直接相加**的手算排序的成績（權重 1:2:3 是手挑的），
> 屬 v1 方法。下面那段註腳自己就寫著同一條排序改用 v2 的零膨脹百分位後是 26.0%。
> **2026-09-12 實測：修正後的 v2 是 26.0%（lift 2.39x），正好落在該註腳預告的數字上。**
> 本規格不降低門檻，但簡報必須說明這兩個數字是不同方法量出來的。
...
1. **`reasons` 至多 3 條、`actions` 至多 3 條。** 派工單是要被人讀完的，不是資料傾倒。
   原訂「恰好 3 條」已放寬為「至少 1 條、至多 3 條」：實測前 50 名裡有 4 園是零裁罰、
   純靠評鑑排上來的，只給得出 2 條真理由。湊第三條等於在派工單上寫一句沒有
   證據的話，而稽查人員會拿它去跟園方對質。
...
| 規則 | 說明 |
|---|---|
| `reasons` 至多 3 條 | 超過 3 條沒有人會讀完。後端負責取前 3 名，前端不再篩選；理由不足 3 條時寧可少寫，不補白話 |
...
- [ ] `features.json` **1,215** 筆，正樣本 128 筆（原訂 1,212 是特徵檔的涵蓋數，
      少的 3 園是仍在營運的非營利園，見 [ADR-0002](adr/0002-資料來源與母體定義的三處修正.md)）
- [ ] 評鑑 join 命中率 100%（1,101 / 1,101）
- [ ] 回測 Precision@50 **≥ 26.0%**（v2 方法的實測上限，見 [ADR-0001](adr/0001-風險分數的權重與缺資料處理.md)；
      §6.3 的 28.0% 是 v1 方法量出來的，兩者不可直接相比）
