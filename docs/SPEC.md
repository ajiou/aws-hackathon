...
risk_score_i = 100 × ECDF_type(R_raw_i)
```

`risk_score` 的語意是「**在同類型園所中的相對風險位置**」，**不是裁罰機率**。
...
**私立一律 `applicable = false`、`score = null`、`Coverage = 0`，不是 0 分。** 私幼依法不需公告財報，把它當 0 分等於懲罰守法者。

#### 公立

```
O_public = 60% × F + 25% × E + 15% × C
```

**F 財務狀況**（內部等權）：總支出預決算偏離（雙尾）、學雜費收入執行率（下尾）、本期短絀／基金來源（上尾）、現金淨減少／期初現金（上尾）、財務結構（負債／資產與淨資產年減率的平均）。

**E 招生狀況**：實際招生／核定人數 50%（下尾）、招生人數年減率 30%（上尾）、超過核定人數 20%（同年度確認超收時該項直接 100 分）。核定與實際年度無法對齊時 `a = 0`。

**C 收費異常**（僅公立）：同儕收費偏離 60%、年齡／學期／半全日班一致性 25%、缺漏或重複項目 15%。
...
```
O_nonprofit = 30% × F + 35% × H + 25% × A + 10% × E
```

**F 財務狀況**（內部等權）：總收入執行率偏離、總支出執行率偏離、短絀／收入、現金減少與流動性、負債比與淨值衰退。

**H 人事執行情形**——這是財報唯一能接上教保風險的橋（見 [`NARRATIVE.md`](NARRATIVE.md) §4.4）：

| 子項 | 權重 | 方向 |
|---|--:|---|
| 人事費總執行率 | 25% | 下尾 |
| 園長及教保人員薪資執行率 | 35% | 下尾 |
| 加班費執行率 | 20% | 上尾（超過預算視為重大缺失） |
| 代課／代班費執行率 | 20% | 上尾及年度異常增加 |

> 低加班費、低代課費**不自動判定為風險**，需看同儕與年度變化。預算為 0 時不除以零，改由查核結果判斷。

**A 查核缺失**：

```
A = 100 × Σ(Severity_j × I(No_j)) / Σ(Severity_j × I(Applicable_j))
```
...
有財報的 280 園中被裁罰過的僅 6 間。**6 個正樣本無法訓練或驗證任何模型。**

因此：

1. 營運權重是**領域判斷手訂的**，不是從資料學出來的
2. `serving/scores.json` 的 `dimensions.operation` 必須帶 `"validated": false`
3. 前端在營運相關的原因與旗標旁顯示「此維度依財務常規判斷，尚無足夠裁罰樣本可驗證」
4. 簡報中不得聲稱營運維度「經過驗證」

**為什麼仍然納入**：營運只影響公立+非營利 347 園，而監督式模型在公立組的 lift 是 0.86x（比隨機還差）。**一個有領域依據但未驗證的訊號，勝過一個已證實無效的訊號。** 這個取捨要明講，不要藏。
...
```json
{
  ...
  "score_basis": "組內百分位（ECDF by institution_type）",
