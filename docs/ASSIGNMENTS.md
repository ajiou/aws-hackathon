...
3. 砍行政區熱力圖，改用地圖點位著色
4. **不可砍**：時間切分回測與效益曲線、稽查派工單、輿情補標（**只補切點前的 1,116 篇**）

> **輿情補標範圍已縮小（2026-09-12 實測後決定）**：原訂補完 2,320 篇。
> 但 `docs.json` 的 2,900 篇裡有 1,540 篇在切點之後，**補那些對 Precision@50
> 毫無貢獻**——特徵只能用切點前的資料。切點前的 1,360 篇目前只標了 244 篇，
> 要補的是剩下的 1,116 篇，1 RPS 下約 19 分鐘。切點後的文件只影響前端
> 「當前輿情」展示，而那一塊可以直接用現成的 `data/media/park_risk.json`。
...
| 風險 | 影響 | 誰負責 |
|---|---|---|
| 輿情 `doc_analysis` 只標了 580/2,900 | 不補標則園級輿情只覆蓋 40 園（3.3%），15% 權重形同虛設 | A，M4，Bedrock 1 RPS **約 19 分鐘**（見下） |
