...
並且用時間切分證明它比隨機抽查準 **2.39 倍**。

> 2.65 倍是 `SPEC.md` §6.3 第 4 列那條**手算基準線**的數字（28.0% ÷ 10.6%），
> 不是模型的成績。模型實測 Precision@50 = 26.0%、lift 2.39x，
> 依據見 [`docs/adr/0001`](docs/adr/0001-風險分數的權重與缺資料處理.md)。
...
| 財報抽表 | ✅ 46/46 份成功 |
| 財報抽表（公立決算 21 園） | ✅ 改由 `docs/總說明/*.md` 抽，61 園所年度、不需解壓 1.5GB 決算書 |
